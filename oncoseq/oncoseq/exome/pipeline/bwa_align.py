...
def reads_alignment(ref_genome, reads_file_name, nmismatch, num_cores, outfile_name, quality_trim):
    args=["bwa",'aln', '-k', str(nmismatch), '-t', str(num_cores), '-q', 
          quality_trim, '-f', outfile_name, ref_genome, reads_file_name]
...
def bwa_alignment(ref_fa, bam_file,read1,read2) #lane, genome, job, config, num_processors, runfunc, deps=None):
    '''
    BWA alignment tasks
    ...
    elif (read1 is not None):
        pass
    else:
        logging.info('Alignment Aborted because there were not reads')
...
def main():
    ...
    parser.add_argument("--read1", dest="read1", default=None)
    parser.add_argument("--read2", dest="read2", default=None)
    parser.add_argument("ref_fa")
    parser.add_argument("bam_file")
    parser.add_argument("pipeline")
    args = parser.parse_args()
    return bwa_alignment(args.ref_fa, args.bam_file,args.pipeline, args.read1,args.read2)
