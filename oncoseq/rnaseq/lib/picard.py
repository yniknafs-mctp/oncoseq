    ...
    def __init__(self, alignment_metrics_file):
        ...
        for line in f:
            if line.startswith("## METRICS"):
                break
        self.header_fields = f.next().strip().split('\t')
...
def get_total_reads(alignment_metrics_file):
    ...
    total_reads_ind = header_fields.index("TOTAL_READS")
    total_reads = 0
    for line in f:
        fields = line.strip().split('\t')
        if fields[0] == "FIRST_OF_PAIR":
            total_reads += int(fields[total_reads_ind])
        elif fields[0] == "SECOND_OF_PAIR":
            total_reads += int(fields[total_reads_ind])
        elif fields[0] == "UNPAIRED":
            total_reads += int(fields[total_reads_ind])
    return total_reads
